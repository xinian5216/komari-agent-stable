...
if [ -n "$source_files" ] && grep -En 'os/exec|exec\.Command|ExecutionPolicy|/api/clients/(terminal|transfer/)' $source_files; then
...
if grep -En 'remote_control_choice|RemoteControlChoice|remote_control_decision|RemoteControlDecision|apply_remote_control_args|Get-ExistingRemoteControlState' install.sh install.ps1; then
    fail 'an installer path can still select a remote-control mode'
fi

if grep -En '"(exec|terminal|file)"' protocol/v2/capabilities.go; then
    fail 'a removed remote-control capability returned'
fi

grep -Fq 'method not supported: remote control is not compiled into this build' server/remote_control.go \
    || fail 'the fail-closed legacy rejection is missing'
grep -Fq 'MarkHidden("disable-web-ssh")' cmd/root.go \
    || fail 'the legacy --disable-web-ssh compatibility flag is not hidden'
grep -Fq 'MarkHidden("disable-remote-control")' cmd/root.go \
