...
if [ -n "${KOMARI_AGENT_REMOTE_CONTROL:-}" ]; then
    log_warning "KOMARI_AGENT_REMOTE_CONTROL is deprecated and ignored: remote control is not compiled into this build"
fi
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        --enable-remote-control)
            log_error "--enable-remote-control is no longer supported: remote control is not compiled into this build"
            exit 1
            ;;
        --disable-remote-control)
            log_warning "--disable-remote-control is a deprecated no-op: this build is always monitoring-only"
            shift
            ;;
        --install*)
...
log_config "  Mode: ${GREEN}monitoring only${NC} (remote control is not compiled into this build)"
