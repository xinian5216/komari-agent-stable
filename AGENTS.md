...
可能很高，因此协议、远程控制、安装器和更新器均按安全敏感代码处理。

## 进入仓库后先读

1. `UPSTREAM.md`：上游基线与 fork 实际差异；
2. `readme.md`：参数、安装、迁移和回滚约定；
3. 按任务读取 `protocol/v2/`、`server/`、`terminal/`、`update/`、安装脚本及对应 CI。
...
- 新安装默认仅监控；远程命令、终端和文件能力必须由用户显式启用。官方 Agent 接管时必须原样保留
  systemd 启动参数、endpoint、token 和既有远控选择。
- token、endpoint、命令内容、终端数据不得写入测试日志、Release 说明或错误回传。
