...
if ($env:KOMARI_AGENT_REMOTE_CONTROL) {
    Log-Warning "KOMARI_AGENT_REMOTE_CONTROL is deprecated and ignored: remote control is not compiled into this build"
}
...
for ($i = 0; $i -lt $args.Count; $i++) {
    switch ($args[$i]) {
        ...
        "--enable-remote-control" {
            Log-Error "--enable-remote-control is no longer supported: remote control is not compiled into this build"
            exit 1
        }
        "--disable-remote-control" {
            Log-Warning "--disable-remote-control is a deprecated no-op: this build is always monitoring-only"
            continue
        }
...
Log-Config "Mode: monitoring only (remote control is not compiled into this build)"
